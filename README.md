...
- **Agent CLIs**: [Claude Code](https://claude.com/claude-code), [Codex](https://developers.openai.com/codex/cli), and [opencode](https://opencode.ai/), each self-managed by its own installer, plus [Cursor CLI](https://cursor.com/cli) from the Brewfile. `bin/cl`, `bin/cx`, `bin/cr` and `bin/oc` wrap Claude Code, Codex, Cursor and opencode.
